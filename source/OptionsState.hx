...
class OptionsState extends MusicBeatState
{
	...
	override function create() {
		...
		menuBG.color = options[curSelected][1];
...
class PreferencesSubstate extends MusicBeatSubstate
{
				...
				switch(options[curSelected]) {
					case 'FPS Counter':
						...
						if(Main.memoryVar != null)
							Main.memoryVar.visible = ClientPrefs.showFPS;
					...
					case 'Reset Key':
						ClientPrefs.resetKey = !ClientPrefs.resetKey;
					case 'Famil Engine Watermarks':
						ClientPrefs.feWatermarks = !ClientPrefs.feWatermarks;
					case 'Instant Respawn':
						ClientPrefs.instantRespawn = !ClientPrefs.instantRespawn;
					case 'Hitsounds':
						ClientPrefs.hitSounds = !ClientPrefs.hitSounds;
					case 'Main Menu Details':
						ClientPrefs.mainMenuDetails = !ClientPrefs.mainMenuDetails;
				}
				FlxG.sound.play(Paths.sound('scrollMenu'));
				reloadValues();
			}
	...
	function changeSelection(change:Int = 0)
	{
		...
		switch(options[curSelected]) {
			...
			case 'Reset Key':
				daText = "If checked, the reset key will be enabled.";
			case 'Famil Engine Watermarks':
				daText = "If checked, Famil Engine logos will appear\ninstead of the neutral ones.";
			case 'Instant Respawn':
				daText = "If checked, you will instantly respawn when\nyou die.";
			case 'Hitsounds':
				daText = "If checked, a sound will play each time you hit a note.";
			case 'Main Menu Details':
				daText = "If unchecked, the characters and other stuff\nwill not appear. Main menu loads faster if unchecked.";
		}
		...
		for (i in 0...checkboxArray.length) {
			...
			if(checkbox != null) {
				...
				switch(options[checkboxNumber[i]]) {
					...
					case 'Reset Key':
						daValue = ClientPrefs.resetKey;
					case 'Famil Engine Watermarks':
						daValue = ClientPrefs.feWatermarks;
					case 'Instant Respawn':
						daValue = ClientPrefs.instantRespawn;
					case 'Hitsounds':
						daValue = ClientPrefs.hitSounds;
					case 'Main Menu Details':
						daValue = ClientPrefs.mainMenuDetails;
				}
