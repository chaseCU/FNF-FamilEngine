...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		if (ClientPrefs.feWatermarks) {
			feWatermarkThing = new FlxText(4, healthBarBG.y
			+ 50, 0,
			SONG.song
			+ " - "
			+ CoolUtil.difficultyFromInt(storyDifficulty)
			+ (" | Famil Engine " + MainMenuState.familEngineVersion), 16);
			feWatermarkThing.setFormat(Paths.font("vcr.ttf"), 16, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
			feWatermarkThing.scrollFactor.set();
		}
		else {
			feWatermarkThing = new FlxText(4, healthBarBG.y
			+ 50, 0,
		...
		add(feWatermarkThing);
		if (ClientPrefs.downScroll) {
			feWatermarkThing.y = FlxG.height * 0.9 + 45;
		}
		...
		feWatermarkThing.cameras = [camHUD];
		doof.cameras = [camHUD];
...
songSpeed = SONG.speed;
	...
	override public function update(elapsed:Float)
	{
		...
		scoreTxt.text = 'Score: ' + songScore + ' | Combo Breaks: ' + songMisses + ' | Accuracy: ' + '\n Rating: ' + ratingString + ' | Combo: ' + combo;
		if(ratingString != '?' && !cpuControlled) {
			scoreTxt.text = 'Score: ' + songScore + ' | Combo Breaks: ' + songMisses + ' | Accuracy: ' + ((Math.floor(ratingPercent * 10000) / 100)) + '%' + '\n Rating: ' + ratingString + ' | Combo: ' + combo;
		} else if (cpuControlled)
			scoreTxt.text = '(Botplay Enabled)';

		if (sicks != 0)
			sicksTxt.visible = true;
		if (goods != 0)
			goodsTxt.visible = true;
		if (bads != 0)
			badsTxt.visible = true;
		if (shits != 0)
			shitsTxt.visible = true;
		...
		if (controls.RESET && !inCutscene && !endingSong && ClientPrefs.resetKey)
		{
			health = 0;
			trace("RESET = True");
		}
		...
		if (generatedMusic)
		{
			...
			notes.forEachAlive(function(daNote:Note)
			{
				...
				if (!daNote.mustPress && daNote.wasGoodHit && !daNote.hitByOpponent && !daNote.ignoreNote)
				{
					...
					if (dad.healthDrains && health > 0.1) {
						health -= dad.healthDrainAmount;
					}
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			...
			if (!note.isSustainNote && ClientPrefs.hitSounds) {
				FlxG.sound.play(Paths.sound('ChartingTick'), 0.5);
			}

			if(note.noteType == 'Danger Note' && !note.isSustainNote) {
				FlxG.sound.play(Paths.sound('shooters'));
			}
