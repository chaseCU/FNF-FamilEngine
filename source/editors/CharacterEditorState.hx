...
class CharacterEditorState extends MusicBeatState
{
	...
	var cameraShakesCheckBox:FlxUICheckBox;
	var characterFloatsCheckBox:FlxUICheckBox;
	var healthDrainsCheckBox:FlxUICheckBox;
	var healthDrainAmount:FlxUINumericStepper;
	function addMiscUI() {
		var tab_group = new FlxUI(null, UI_box);
		tab_group.name = "Misc";

		cameraShakesCheckBox = new FlxUICheckBox(15, 12, null, null, "Camera Shakes When Singing", 80);
		cameraShakesCheckBox.checked = char.cameraShakes;
		cameraShakesCheckBox.callback = function() {
			char.cameraShakes = cameraShakesCheckBox.checked;
		};

		characterFloatsCheckBox = new FlxUICheckBox(cameraShakesCheckBox.x, cameraShakesCheckBox.y + 40, null, null, "Character Floats", 80);
		characterFloatsCheckBox.checked = char.charFloats;
		characterFloatsCheckBox.callback = function() {
			char.charFloats = characterFloatsCheckBox.checked;
		};

		healthDrainsCheckBox = new FlxUICheckBox(characterFloatsCheckBox.x, characterFloatsCheckBox.y + 40, null, null, "Health Drains When Singing", 80);
		healthDrainsCheckBox.checked = char.charFloats;
		healthDrainsCheckBox.callback = function() {
			char.healthDrains = healthDrainsCheckBox.checked;
		};

		healthDrainAmount = new FlxUINumericStepper(healthDrainsCheckBox.x + 10, healthDrainsCheckBox.y + 38, 0.1, 0, 0, 1, 0);
		healthDrainAmount.width + 100;
		healthDrainAmount.visible = healthDrainsCheckBox.checked;

		var hDAText:FlxText = new FlxText(healthDrainsCheckBox.x, healthDrainAmount.y - 18, 0, 'Health Drain Amount:');
		hDAText.visible = healthDrainsCheckBox.checked;

		tab_group.add(hDAText);

		tab_group.add(cameraShakesCheckBox);
		tab_group.add(characterFloatsCheckBox);
		tab_group.add(healthDrainsCheckBox);
		tab_group.add(healthDrainAmount);
		UI_characterbox.addGroup(tab_group);
	}
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>) {
		if(id == FlxUIInputText.CHANGE_EVENT && (sender is FlxUIInputText)) {
			...
			else if(sender == healthDrainAmount)
			{
				char.healthDrainAmount = healthDrainAmount.value;
			}
		}
	}
	...
	function saveCharacter() {
		var json = {
			...
			"camera_shakes": char.cameraShakes,
			"char_floats": char.charFloats,
			"health_drains": char.healthDrains,
			"health_drain_amount": char.healthDrainAmount,
